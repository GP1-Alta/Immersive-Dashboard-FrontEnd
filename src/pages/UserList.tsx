...
const UserList = () => {
  ...
  const addNewUser = () => {
    axios
      .post("http://34.123.29.56/register", {
        name: `${newName}`,
        email: `${newEmail}`,
      ...
      .then((res) => {
        ...
        axios
          .get(`http://34.123.29.56/users?page=${page}&key=${search}`, {
            headers: {
  ...
  const deleteUser = (id: number) => {
    console.log(id);
    axios
      .delete(`https://virtserver.swaggerhub.com/KHARISMAJANUAR/api-immersive-dashboard/1.0.0/users/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
            ...
            <label className="modal-box relative w-screen" htmlFor="">
              ...
              <div className="grid justify-items-center w-full">
                <div className="flex w-full gap-5 justify-end">
                  ...
                  <label htmlFor="my-modal-4" onClick={addNewUser} className="btn bg-[#19345E] w-20 flex gap-2">
                    Save
                  </label>
                </div>
              </div>
